...
it('creates lesson with a material',async()=>{vi.spyOn(globalThis,'fetch').mockResolvedValueOnce(new Response(JSON.stringify({items:[]}),{status:200})).mockResolvedValueOnce(new Response(JSON.stringify({items:[]}),{status:200})).mockResolvedValueOnce(new Response(JSON.stringify({items:[]}),{status:200})).mockResolvedValueOnce(new Response(JSON.stringify({lesson:{id:'l-1',classSubjectId:'link-1',lessonDate:'2026-09-09',position:1,topic:'Дроби',homework:null,materials:[{id:'m-1',title:'Видео',url:'https://example.test',position:1}],gradeItems:[]}}),{status:201}));render(<TeacherLessonsView assignment={assignment} quarters={[{id:'q-1',number:1,startsOn:'2026-09-09',endsOn:'2026-09-09'}]} onBack={()=>{}}/>);await userEvent.click(await screen.findByRole('button',{name:'Домашнее задание 2026-09-09'}));await userEvent.clear(screen.getByLabelText('Дата урока'));await userEvent.type(screen.getByLabelText('Дата урока'),'2026-09-09');await userEvent.type(screen.getByLabelText('Тема'),'Дроби');await userEvent.click(screen.getByRole('button',{name:/Материал/}));await userEvent.type(screen.getByLabelText('Название материала 1'),'Видео');await userEvent.type(screen.getByLabelText('Ссылка материала 1'),'https://example.test');await userEvent.click(screen.getByRole('button',{name:'Сохранить'}));expect(await screen.findByText('Дроби')).toBeInTheDocument()})
it('shows one week by default and saves the period view in a cookie',async()=>{
 vi.spyOn(globalThis,'fetch')
  .mockResolvedValueOnce(new Response(JSON.stringify({items:[]}),{status:200}))
  .mockResolvedValueOnce(new Response(JSON.stringify({items:[]}),{status:200}))
  .mockResolvedValueOnce(new Response(JSON.stringify({items:[]}),{status:200}))
 render(<TeacherLessonsView assignment={assignment} quarters={[{id:'q-1',number:1,startsOn:'2000-01-01',endsOn:'2000-01-03'}]} onBack={()=>{}}/>)
