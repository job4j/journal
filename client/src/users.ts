...
export interface UserRecord{id:string;login?:string;email?:string|null;phone?:string|null;firstName:string;lastName:string;status:'active'|'blocked';roles:string[]}
export interface UserDraft{login:string;email:string;phone:string;password:string;firstName:string;lastName:string;status:'active'|'blocked';roles:string[]}
...
export async function createUser(draft:UserDraft):Promise<UserRecord>{const body={login:draft.login,email:draft.email||null,phone:draft.phone||null,password:draft.password,firstName:draft.firstName,lastName:draft.lastName,roles:draft.roles};return(await request<{user:UserRecord}>('/api/v1/users',{method:'POST',body:JSON.stringify(body)})).user}
