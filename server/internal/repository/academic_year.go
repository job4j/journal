...
func (r *Repository) ListAcademicYears(ctx context.Context, tx Transaction) ([]entity.AcademicYear, error) {
	return queryMany[entity.AcademicYear](ctx, tx, "list academic_years", `SELECT id, name, starts_on, ends_on, status, created_at, updated_at FROM academic_years ORDER BY created_at, id`)
}
