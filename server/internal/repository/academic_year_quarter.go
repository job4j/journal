...
func (r *Repository) ListAcademicYearQuarters(ctx context.Context, tx Transaction) ([]entity.AcademicYearQuarter, error) {
	return queryMany[entity.AcademicYearQuarter](ctx, tx, "list academic_year_quarters", `SELECT id, academic_year_id, number, starts_on, ends_on, created_at, updated_at FROM academic_year_quarters ORDER BY created_at, id`)
}
